...
//deprecate                .signWith(SignatureAlgorithm.HS512, jwtSecret)
    ...
    public String getUserNameFromJwtToken(String token) {
        ...
        //deprecate        return Jwts.parser().setSigningKey(jwtSecret).parseClaimsJws(token).getBody().getSubject();
    }

    public boolean validateJwtToken(String authToken) {
        try {
            Jwts.parserBuilder().setSigningKey(jwtSecret).build().parseClaimsJws(authToken);
            //deprecate     Jwts.parser().setSigningKey(jwtSecret).parseClaimsJws(authToken);
            return true;
